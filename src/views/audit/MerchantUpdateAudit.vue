...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      try {
        var res = await merchantAuditService.merUpAuditList(data)
        ...
        console.error(err)
        this.$notify.error({
          titile: '错误', message: '查询商户变更审核信息时发生错误'
        })
      }
    },
    handleAudit(index, row) {
      this.$router.push({ path: '/audit/merchantUpdateAudit/MerchantUpdateAuditDetail', query: {
        applyId: row.applyId,
