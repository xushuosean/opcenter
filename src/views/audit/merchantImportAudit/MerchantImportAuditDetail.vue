<template>
  <div class="box">
    ...
    <el-card>
      <h3>商户结算信息</h3>
      <AuditWrapTable :table-data="settlementInfoData" :show-change="false" />
    </el-card>
    <el-card>
      <h3>签约交易类型</h3>
    ...
    <el-card>
      <h3>图片资料上传</h3>
      <AuditWrapTable :table-data="imgData" />
    </el-card>
    <el-card>
      <h3>新增商户下属门店</h3>
      <div v-for="(item , index) in shopData" :key="index">
        <el-divider />
        <AuditWrapTable
          :table-data="item.baseShopinfo"
        />
        <h4>门店管理员账号</h4>
        <AuditWrapTable
          :table-data="item.shopManager"
        />
        <h4>结算信息</h4>
        <div v-if="item.sourceData.isSyncMechtSettle">
          同步所属商户结算信息
        </div>
        <AuditWrapTable v-else :table-data="item.settlementInfo" :show-change="false" />
      </div>
    </el-card>
    <!-- 审核日志 -->
  ...
  data() {
    return {
      ...
      settlementInfo: [
        ...
        { label: '账户类型', keyEnum: 'acctType', template: 'enum' },
