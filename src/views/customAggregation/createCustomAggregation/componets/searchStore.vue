  ...
  <div class="shiyongmendian">
    <el-form>
      ...
      <template v-if="formData.importShopStyle==='batchImport'">
        <el-button type="primary" class="batchImportButton" @click="dowloadExcel">
          下载Excel模板
        </el-button>
        <el-upload
          class="upload-demo"
          ...
          :file-list="fileList"
        >
          <el-button type="primary" class="batchImportButton" @click="updataCoupon">
            上传门店名单
          </el-button>
        ...
        <div class="manualInput">
          <el-form-item prop="searchMethod" style="margin:20px 0">
            <el-select v-model="formData.searchMethod">
              <el-option label="搜索单个门店" value="byStore" />
              <el-option label="按门店地区及类别筛选" value="byArea" />
            </el-select>
          </el-form-item>
          <!-- 按照名字或者编号搜索 -->
          <template v-if="formData.searchMethod==='byNameOrCode'||'byStore'">
...
<script>
import TicketsManage from '@api/discountPaymentManage.service.js'
...
export default {
  ...
  methods: {
    ...
    async searchShopbyAreaCat() {
      ...
      data = objectTool.searchShop(data)
      try {
        let res = await TicketsManage.searchShop(data)
        if (res.resultCode === AjaxReturnType.SUCCESS) {
          this.searchedShopList = res.data
    ...
    dowloadExcel() {
      ...
      window.location.href = origin + '/' + pathname.split('/')[1] + '/common/poly/shop/templateDownload.json'
    }
  }
}
</script>
