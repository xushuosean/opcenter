<template>
  <el-container class="pageContainer">
    <el-header>
      <h3>商圈管理 <el-button @click="newAreaVisible=true">+新增商圈</el-button></h3>
    </el-header>
    <el-container>
      <el-aside>
        <!-- 左边栏选择树 -->
      ...
      <el-main>
        <div class="title">
          ...
          <p>该层级下共有{{ areaCount }}个子商圈</p>
        </div>
        <el-table :data="tableData" border>
          <el-table-column prop="districtId" label="商圈编号" />
          <el-table-column prop="districtName" label="商圈名称" />
          <el-table-column prop="status" label="商圈状态" />
          <!-- 表格的按钮栏目 -->
          <el-table-column
            fixed="right"
            width="160"
      ...
      <!-- 商圈编辑对话框 -->
      <el-dialog title="商圈编辑" :visible.sync="editAreaVisible" width="500px">
        <el-form ref="editArea" :model="editAreaForm" :rule="editAreaRule">
          <el-form-item label="商圈编号">
      ...
      <!-- 删除按钮对话框 -->
      <el-dialog :visible.sync="deleteAreaVisible" title="删除商圈" width="500px">
        <el-row>
          <el-col :span="6">商圈名称</el-col>
          <el-col :span="16"> {{ deleteAreaForm.districtName }}</el-col>
        </el-row>
        <el-row>
          <el-col :span="6">商圈编号</el-col>
  ...
  methods: {
    ...
    async getTableData() {
      ...
      try {
        ...
        if (res.resultCode === AjaxReturnType.SUCCESS) {
          ...
          throw new Error('查询商圈列表失败')
        }
      } catch (err) {
        console.error(err)
        this.$message.error({ message: '查询商圈列表失败' })
      }
    },
    ...
    async affirmNewArea() {
      ...
      try {
        ...
        if (res.resultCode === AjaxReturnType.SUCCESS) {
          this.$message.success({ message: '新增商圈成功' })
        } else {
          throw new Error('新增商圈失败')
        }
      } catch (err) { console.error(err); this.$message.error({ message: '新增商圈失败' }) } finally {
        this.$refs.newArea.resetFields()
        this.newAreaVisible = false
      }
    },
    ...
    async affirmEditArea() {
      ...
      try {
        ...
        if (res.resultCode === AjaxReturnType.SUCCESS) {
          this.$message.success({ message: '编辑商圈成功' })
        } else {
          throw new Error('编辑商圈失败')
        }
      } catch (err) { console.error(err); this.$message.error({ message: '编辑商圈失败' }) } finally {
        this.$refs.editArea.resetFields()
        this.editAreaVisible = false
      }
    },
    ...
    async affirmDelete() {
      try {
        ...
        if (res.resultCode === AjaxReturnType.SUCCESS) {
          this.$message.success({ message: '商圈删除成功' })
        } else { throw new Error('删除商圈失败') }
      } catch (err) {
        console.error(err)
        this.$message.error({ message: '删除商圈失败' })
      } finally {
        this.deleteAreaVisible = false
      }
    },
