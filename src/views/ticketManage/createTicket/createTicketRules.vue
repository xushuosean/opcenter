  ...
  <div class="container-box">
    <el-card class="el-card-mt20" shadow="hover">
      ...
      <el-form label-position="left" label-width="140px" class="componentForm el-card-mt20" size="medium">
        ...
        <el-form-item label="允许购买时间">
          <el-select v-model="componentForm.timeConfList[2].setAppointTime" :clearable="true" placeholder="请选择按钮类型">
            <el-option label="与展示时间相同" value="true" />
            <el-option label="指定时间段" value="false" />
          </el-select>
        </el-form-item>
        <el-form-item v-show="componentForm.timeConfList[2].setAppointTime == 'false'">
          自
          <el-date-picker
            v-model="componentForm.timeConfList[2].startTime"
            type="datetime"
            placeholder="选择日期时间"
          />
        </el-form-item>
        <el-form-item v-show="componentForm.timeConfList[2].setAppointTime == 'false'">
          至
          <el-date-picker
            v-model="componentForm.timeConfList[2].endTime"
            type="datetime"
            placeholder="选择日期时间"
          />
        </el-form-item>
        <el-form-item>
          <el-checkbox v-model="componentForm.timeConfList[2].setCycle">设置循环周期</el-checkbox>
        </el-form-item>
        <el-form-item v-show="componentForm.timeConfList[2].setCycle">
          每周
        ...
        <el-form-item v-show="componentForm.timeConfList[2].setCycle">
          <el-time-picker
            v-model="componentForm.timeConfList[2].cycleStartTime"
            placeholder="任意时间点"
          />
          至
          <el-time-picker
            v-model="componentForm.timeConfList[2].cycleEndTime"
            placeholder="任意时间点"
          />
        </el-form-item>
        <el-form-item v-show="componentForm.timeConfList[2].setCycle">
          ...
          <el-input v-show="componentForm.setSubStock" v-model="componentForm.timeConfList[2].subStockNum" style="width:100px;margin:0 5px" /><span v-show="componentForm.setSubStock">张(最多X张)</span>
        </el-form-item>
        <el-form-item v-show="componentForm.timeConfList[2].setCycle">
    ...
    <el-card class="el-card-mt20" shadow="hover">
      ...
      <el-form label-position="left" label-width="140px" class="componentForm el-card-mt20" size="medium">
        <el-form-item label="补贴结算模式">
          <el-select v-model="dealType" :clearable="true" placeholder="请选择按钮类型">
            <el-option label="见票前结算" value="before" />
            <el-option label="指定时间段" value="aim" />
          </el-select>
        </el-form-item>
        <el-form-item label="分摊规则">
          ...
          <div>
            总行 <el-input v-model="componentForm.bankSubsidyAmt" style="width:100px;margin:0 5px" placeholder="补贴金额" />元
          </div>
          <div class="mt10">
            分行 <el-input v-model="componentForm.branchSubsidyAmt" style="width:100px;margin:0 5px" placeholder="补贴金额" />元
          </div>
          <div class="mt10">
            商户 <el-input v-model="componentForm.mchtSubsidyAmt" style="width:100px;margin:0 5px" placeholder="补贴金额" />元
          </div>
        </el-form-item>
        <el-form-item label="结算方式">
          <el-select v-model="componentForm.settleWay" :clearable="true" placeholder="请选择按钮类型">
            <el-option label="按核销结算(用户可退款)" value="VERIFY" />
            <el-option v-show="achievetype== 'BUY'" label="按购买结算(用户不可退款)" value="BUY" />
          </el-select>
        </el-form-item>
      </el-form>
    </el-card>
  ...
  data() {
    return {
      ...
      componentForm: {
        ...
        subLimitType: '',
        subLimitNum: '', // 子限购类型   DAY-每天 WEEK-每周
        bankSubsidyAmt: '', // 总行补贴
  ...
  watch: {
    componentForm: {
      handler: function(newVal, oldVal) {
        this.$emit('getForm2data', this.componentForm)
      },
      deep: true
    },
    ...
    'checkForm1.checkedList'() {
      ...
      this.componentForm.timeConfList[2].cycleContent = a
    },
